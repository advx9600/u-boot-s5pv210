...
void realarm_radio_config(void)
{
	...
	char *p;
	char save_cmdline[256]="setenv bootargs ";
	char cmdline_temp[256];
...
radio_config:
	...
	switch(select) 
	{
		...
		case '2':
		case '3':
			p = strstr(cmdline,"radio");
			if(p)
			{
				while((*p!=' ') && (*p!= 0))
					p++;

				strcpy(cmdline_temp, p);
				...
				if(p > cmdline)
					...
					*p = 0;
				strcat(cmdline, ppp_dev[select-'1']);
				strcat(cmdline, cmdline_temp);
			}
			else
			{
				strcat(cmdline, ppp_dev[select-'1']);
			}
			strcat(save_cmdline, cmdline);
//			sprintf(cmdline + strlen(cmdline), "%s", ppp_dev[select-'1']);
			ExecuteCmd(save_cmdline);
			ExecuteCmd("saveenv");
			break;
...
void realarm_calibration_config(void)
{
	char *p;
	char save_cmdline[256]="setenv bootargs ";
	char cmdline_temp[256];
	p = strstr(cmdline,"skipCalibrate");
	if(p)
	{
		while((*p!=' ') && (*p!= 0))
			p++;
...
void RealARMMenu(void)
{
	...
	while(1) {
		...
		if(strstr(cmdline,"skipCalibrate"))
			printf("[t] Touchscreen calibration not skip\n");
		else
			printf("[t] Touchscreen calibration skip\n");	
		...
		{		
			...
			//[t] Touchscreen Calibration Skip
			case 'T': case 't':
				realarm_calibration_config();
				break;		
