...
int realv210_init(void)
{
	...
	writel(readl(GPJ2DAT) & ~(1<<5), GPJ2DAT);
	//udelay(2000000);
	//writel(readl(GPJ2DAT) & ~(1<<6), GPJ2DAT);

	/**************************wifi bluetooth reset**********************************************/
	reg = readl(GPJ3CON) & 0x000ff000;
	writel(reg | 0x00011000, GPJ3CON);

	reg = readl(GPJ2CON) & 0x000f0000;
	writel(reg | 0x00010000, GPJ2CON);
	
	writel(readl(GPJ2DAT)  | (1<<4), GPJ2DAT);
	writel(readl(GPJ3DAT)  | (1<<4), GPJ3DAT);		
	udelay(100000);	
	writel(readl(GPJ2DAT) & ~((1<<4)), GPJ2DAT);	
	writel(readl(GPJ3DAT) & ~((1<<4)), GPJ3DAT);	
	udelay(100000);
